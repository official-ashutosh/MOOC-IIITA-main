...
const registerUser = async (req, res) => {
  ...
  if (user) {
    ...
    try {
      // bcrypt.genSalt() creates a unique, random string that makes the hashed password
      const salt = await bcrypt.genSalt();
      // Hashing is One Way Function.
      // bcrypt.hash(password, salt) takes the user's password and combines
      // it with the salt, then creates a hashed (encoded) version of the password. 
      const hashed = await bcrypt.hash(password, salt);

      //generate otp(means create otp having length 6 and only digits).
      ...
      var mailOptions = {
        from: `MOOC@ IIITA​" <${process.env.EMAIL_USER}>`,
        to: `${email}`,
        subject: "Your OTP to verify your email",
        html: `
        <h1>Hello ${name}!</h1>
        <p>Thank you for registering with MOOC@IIITA! To verify your email address, please use the OTP code below:</p>
        <h2>OTP: ${otp}</h2>
        <p>This OTP is valid for 5 minutes. If you did not request this verification, please ignore this email.</p>
        <p>Best regards,<br>MOOC Team.</p>
      `,
      };
