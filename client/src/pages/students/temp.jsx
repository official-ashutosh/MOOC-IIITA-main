...
import { createStripeSession } from "../../services/paymentService"; // Your backend API to create a session

// Load your Stripe public key
const stripePromise = loadStripe("your-public-stripe-key");

const CheckoutPage = () => {
  const { cartItems, setCartItems, setItemCount } = useContext(CartContext);
  const [totalPrice, setTotalPrice] = useState(0);
  const [paymentMethod, setPaymentMethod] = useState(null); // Changed default to null
  const navigate = useNavigate();
  const stripe = useStripe();  // Use Stripe hook
  const elements = useElements();  // Use Elements hook for Stripe components
  ...
  const handleStripeCheckout = async () => {
    if (!stripe || !elements) {
      return; // Stripe.js has not loaded yet, so no action
    }

    try {
      const updatedCart = await getCart();
      const lineItems = updatedCart.courseDetails.map(item => ({
        price: item.course.stripePriceId, // Ensure you store Stripe price ID on your course items
        quantity: 1,
      }));
      ...
      if (error) {
        toast.error("Error redirecting to Stripe: " + error.message);
      }
    } catch (error) {
